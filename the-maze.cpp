...
public:
  ...
  bool hasPath(vector<vector<int>> &maze, vector<int> &start,
    ...
    // auto is_gt = [&cost]( const Trajectory & a, const Trajectory & b ) ->
    // bool {
    auto is_gt = [](const Trajectory &a, const Trajectory &b) -> bool {
    ...
    for (; !q.empty();) {
      ...
      for (auto dir : vector<int>{UP, RIGHT, DOWN, LEFT}) {
        ...
        vector<int> dest(2, -1);
        if (exploreInOneDirection(maze, visited, ball, destination, dir, dest,
                                  cost)) {
...
protected:
  ...
  static bool exploreInOneDirection(const vector<vector<int>> &maze,
    ...
    if (n != 0) {
      ...
      //			if ( costAt( cost, ball ) + n >= costAt( cost,
      // vector<int>({row,col}) ) ) { 				return false;
      //			}
      //
      dest[0] = row;
      dest[1] = col;
