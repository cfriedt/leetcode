...
public:
  ...
  uint64_t toInteger(ListNode *l) {
    ...
    uint64_t accu;
    uint64_t factor;
    for (accu = 0, factor = 1, it = l; nullptr != it;
         accu += factor * it->val, it = it->next, factor *= 10)
      ;
    return accu;
  }
  ...
  ListNode *toList(uint64_t x) {
    ...
    ListNode *root, *it;
    for (root = new ListNode(x % 10), it = root; 0 != x; x /= 10,
        it->next = (0 == x) ? nullptr : new ListNode(x % 10), it = it->next)
      ;
    return root;
  }
  ...
  ListNode *addTwoNumbers(ListNode *l1, ListNode *l2) {
    ...
    // The numbers should be added together and returned as a linked list, also
    // in reverse order.
    ...
    if (len1 >= 19 || len2 >= 19) {
      ...
      int carry = 0;
      ListNode *root, *it;
      for (root = new ListNode(0), it = root;
