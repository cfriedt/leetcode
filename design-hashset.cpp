...
public:
  ...
  MyHashSet() : n_items(0), buckets(bucket_block_size, nullptr) {}

  void add(int key) { add(key, nullptr); }
...
protected:
  ...
  struct bucket_list_node {
    ...
    explicit bucket_list_node(int key)
        : key(key), hashed_key(hasher(key)), next(nullptr) {}
  };
