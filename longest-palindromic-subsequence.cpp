...
protected:
	int lcs( const string & x, const string & y ) {
		...
		// MAJOR PROBLEM with going from i = 1:M, j = 1:N though, as shown on the wikipedia:
		// there is a "sometimes off by 1" error!!
		// Rather than try to find the strange corner-case that fixes the off-by-one
		// error, going from i=M:1, j=N:1 seems to avoid it altogether.
		...
		for( size_t i = M - 1; i != size_t(-1); i-- ) {
			for( size_t j = N - 1; j != size_t(-1); j-- ) {
				...
				if ( x[ i ] == y[ j ] ) {
					...
					//                    = max( max( LCS( a, a ), LCS( ab, ) ), max( LCS( a, a ), LCS( ,ac ) ) )
