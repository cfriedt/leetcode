...
protected:
	...
	void helper( const vector<vector<int>> & maze, const vector<int> & ball, const vector<int> & hole, int prev_dir, vector<vector<bool>> & visited, vector<vector<int>> & cost, string path, string & shortest ) {

		//cerr << "(" << ball[ 0 ] << "," << ball[ 1 ] << ") path " << path << endl;

		if ( ball == hole ) {

			if ( "" == shortest ) {
//				cerr
//					<< "path " << path << " (" << path.size() << ") beats" << endl
//					<< "shortest " << shortest << " (" << shortest.size() << ")" << endl
//					;
				shortest = path;
				return;
			}
...
//					;
		...
		struct EdgeData {
			...
			EdgeData( int cumulative_cost, int edge_cost, int dir, const vector<int> & dest )
				: cumulative_cost( cumulative_cost ), edge_cost( edge_cost ), dir( dir ), dest( dest )
			{}
		};
		...
		vector<int> dest = ball; // location of next cell (typically against a wall, or over the hole)
		...
		for( auto dir: vector<int>({ UP, RIGHT, DOWN, LEFT }) ) {
			...
			if ( exploreInOneDirection( maze, visited, ball, hole, dir, dest, cost ) ) {
				EdgeData ed( costAt( cost, dest ), costAt( cost, dest ) - costAt( cost, ball ), dir, dest );
				q.push_back( ed );
		...
		for( ; ! q.empty() ;  ) {
			...
			helper( maze, e.dest, hole, e.dir, visited, cost, path + string( e.edge_cost, dirToChar( e.dir ) ), shortest );
		}
	}
...
//		}
	...
	static inline bool isHole( const int row, const int col, const vector<int> & hole ) {
