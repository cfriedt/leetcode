...
protected:
    ...
    struct ent {
        ...
        ent( int time, int candidate, int leader, int leader_count /* , unordered_map<int,int> tally  */ )
        : time( time ), candidate( candidate ), leader( leader ), leader_count( leader_count ) /* , tally( tally ) */
        {}
    };
...
public:
    ...
    : times( times )
    {
        ...
        for( size_t i = 0; i < N; i++ ) {
            ...
            if ( tally[ candidate ] >= leader_count ) {
                ...
                leader = candidate;
                //cerr << "candidate " << candidate << " took the lead at time " << times[ i ] << " with a score of " << leader_count << endl;
            }
    ...
    int q(int t) {
        ...
        for( int L = 0, U = times.size() - 1; L <= U; ) {
            int M = (L + U) / 2;
            int direction = compare( t, times[ M ], (M < (int)times.size() - 1) ? times[ M + 1 ] : -1 );
            if ( 0 == direction ) {
                i = M;
                break;
            }
...
protected:
    ...
    static unordered_map<int,int> add(const unordered_map<int,int> & a, const unordered_map<int,int> & b) {
